...
function PayModal({ ar, onClose, onSaved }) {
  const [form, setForm]     = useState({
    bank_account_id: '', amount: ar.pending_amount,
  ...
  async function submit(e) {
    ...
    try {
      ...
      await treasuryApi.payReceivable(ar.id, payload);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{ar.third_party?.business_name}</p>
          </div>
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg flex items-center justify-between">
            ...
            <span className="text-lg font-bold font-mono text-gray-900 dark:text-white">{formatCOP(ar.pending_amount)}</span>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="number" step="0.01" min="0.01" max={ar.pending_amount}
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [listRes, agingRes] = await Promise.all([
        treasuryApi.getReceivables({ page, per_page: 20, status: status || undefined, search: search || undefined }),
        treasuryApi.getReceivableAging(),
      ]);
    ...
    <LayoutNew>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Cuentas por Cobrar</h1>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-32 gap-2">
              ...
              <p className="text-sm text-gray-400">Sin cuentas por cobrar</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-700/50">
                    {['Cliente', 'Documento', 'Vencimiento', 'Días', 'Total', 'Pagado', 'Pendiente', 'Estado', ''].map(h => (
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {rows.map(ar => (
                    <tr key={ar.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 group">
                      <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white max-w-[160px] truncate">
                        {ar.third_party?.business_name ?? '—'}
                      </td>
