...
import { Plus, Search, X, ClipboardList, ChevronLeft, ChevronRight, CheckCircle, RotateCcw } from 'lucide-react';
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <button
            ...
            onClick={() => showError('Próximamente: crear comprobante')}
          >
        ...
        {loading ? (
          ...
          <div className="card py-16 text-center">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">
              {hasFilters ? 'Ajusta los filtros de búsqueda' : 'Los comprobantes aparecen aquí cuando se crean ventas o se ingresan manualmente'}
            </p>
          </div>
        ) : (
          <>
            <div className="card overflow-hidden p-0">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {items.map((v) => (
                    <tr key={v.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors group">
                      ...
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className="text-xs text-gray-600 dark:text-gray-400">
                          {v.date ? new Date(v.date + 'T00:00:00').toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3 hidden lg:table-cell max-w-xs">
                        <p className="text-xs text-gray-600 dark:text-gray-400 truncate">{v.description ?? '—'}</p>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {v.status === 'draft' && (
                            <button
                              ...
                              title="Contabilizar"
                              className="p-1.5 hover:bg-emerald-100 dark:hover:bg-emerald-900/30 rounded text-gray-500 hover:text-emerald-600 transition-colors"
                            >
                          ...
                          {v.status === 'posted' && (
                            <button
                              ...
                              title="Anular"
                              className="p-1.5 hover:bg-red-100 dark:hover:bg-red-900/30 rounded text-gray-500 hover:text-red-500 transition-colors"
                            >
